...
class CommentDaoTest {
    ...
    void shouldGetCommentList() {
        Comment comment = em.find(Comment.class, FIRST_COMMENT_ID);
        List<Comment> commentsExcepted = commentDao.findAllByBookId(comment.getId());
    ...
    void doesntShouldGetCommentList() {
        List<Comment> commentsExcepted = commentDao.findAllByBookId(24);
    ...
    void shouldDeleteCommentById() {
        Comment comment = em.find(Comment.class, FIRST_COMMENT_ID);
        boolean rs = commentDao.deleteById(comment.getId());
